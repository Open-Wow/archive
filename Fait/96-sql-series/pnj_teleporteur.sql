...
SET @GOSSIP := [ID DE TON GOSSIP];
SET @MAP := [ID DE LA MAP];
SET @X := [POSITION X];
SET @Y := [POSITION Y];
SET @Z := [POSITION Z];
SET @O := [ORIENTATION];
SET @TP := [LE NOM DE TON TP];
...
(@ENTRY,0,2,0,62,0,100,0,@GOSSIP,0,0,0,62,@MAP,0,0,0,0,0,7,0,0,0,@X,@Y,@Z​,@O,"Téléportation à @TP");